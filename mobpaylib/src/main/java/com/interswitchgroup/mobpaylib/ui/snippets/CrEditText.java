...
public class CrEditText extends AppCompatEditText {
  ...
  @Nullable private String mSeparator;
  ...
  private void applyAttributes(AttributeSet attrs) {
    TypedArray a =
        getContext().getTheme().obtainStyledAttributes(attrs, R.styleable.CrEditText, 0, 0);
    try {
      setSeparator(
          Separator.values()[a.getInt(R.styleable.CrEditText_separator, Separator.NONE.ordinal())]);
      //            setDisabledCardsInternal(a.getInt(R.styleable.EditCredit_disabledCards, 0));
      setDrawableGravity(
          Gravity.values()[
              a.getInt(R.styleable.CrEditText_drawableGravity, Gravity.END.ordinal())]);
    } finally {
      a.recycle();
    }
  }
      ...
      switch (mDrawableGravity) {
        case START:
          TextViewCompat.setCompoundDrawablesRelativeWithIntrinsicBounds(
              this, currentDrawable, null, null, null);
          break;
        case RIGHT:
        ...
        default:
          TextViewCompat.setCompoundDrawablesRelativeWithIntrinsicBounds(
              this, null, null, currentDrawable, null);
          break;
      }
    }
  }
  ...
  private void addSeparators() {
    ...
    if (mSeparator != null) {
      if (text.length() > 4 && !text.matches("(?:[0-9]{4}" + mSeparator + ")+[0-9]{1,4}")) {
        ...
        if (caretPosition < text.length()) setSelection(caretPosition);
      }
    }
  }
  ...
  /**
   * Use this method to set the separator style. The default separator is {@link Separator#NONE}.
   *
  ...
  /**
   * Use this method to set the location of the card drawable. The default gravity is {@link
   * Gravity#END}.
   *
