...
public class NetModule {
  ...
  @Singleton
  Gson provideGson() {
    return new GsonBuilder().setLenient().setDateFormat("yyyy-MM-dd hh:mm:ss.S").create();
  }
              ...
              try {
                ...
                String encodedClientId =
                    Base64.encodeToString(
                        clientId.getBytes(StandardCharsets.UTF_8), Base64.NO_WRAP);
                String httpMethod = original.method();
                String url = original.url().toString();
                String encodedUrl = URLEncoder.encode(url, "UTF-8");
                String[] signatureItems = {
                  httpMethod, encodedUrl, timestamp, nonce, clientId, clientSecret
                };
                ...
                String signature =
                    Base64.encodeToString(
                        messageDigest.digest(signatureCipher.getBytes(StandardCharsets.UTF_8)),
                        Base64.NO_WRAP);
